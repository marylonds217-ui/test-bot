...
class HWIDBan(commands.Cog):
    ...
    @commands.command(name="hwidban", aliases=["banhwid", "حظر_هردوير"])
    @commands.has_permissions(administrator=True)
    @check_permission("hwidban")
    async def hwidban(self, ctx, *, user_input=None):
        """حظر Hardware ID - !hwidban @user [reason]"""
        ...
        await db.add_hwid_ban(member.id, ctx.guild.id, reason, ctx.author.id)
        
        # حظر العضو عادياً
        try:
            await member.ban(reason=f"HWID BAN: {reason} - Banned by {ctx.author}")
            
            embed = discord.Embed(
                title=f"{EMOJIS['ban']} HWID Ban",
                description=f"{member.mention} has been Hardware ID banned.",
                color=COLORS["ban"]
            )
